...
def solve (N, start, finish, Ticket_cost):
    start, finish = min(start, finish), max(start, finish)
    cost1 = cost2 = 0
    # clockwise direction
    cost1 = sum(Ticket_cost[start-1:finish-1])
    # anticlockwise direction
    cost2 = sum(Ticket_cost[:start-1])
    cost2 += sum(Ticket_cost[finish-1:])
...
You are given an integer N where N represents the total number of the tram stations, an integer start which represents 
the start station, and an integer finish which represents the finish station. You are given an array of positive integers  
ticket_cost where ticket_cost[i] represents the ticket cost between the station number i and (i + 1) % N.
